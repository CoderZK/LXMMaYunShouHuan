...
-(void)openImagePickerWithAction:(OpenImagePickerAction)action completed:(void(^)(UIImage * image,NSString * filePath))completed
{
    if (!self.snyImgPicker)
    {
        ...
        self.snyImgPicker.navigationBar.translucent= NO;
        self.snyImgPicker.modalPresentationStyle = UIModalPresentationFullScreen;
    }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [selfWeak dismissViewControllerAnimated:YES completion:^{
        ...
        if ([mediaType isEqualToString:(NSString *)kUTTypeImage])
        {
            ...
            if (selfWeak.snycompleted)
            {
                selfWeak.snycompleted(image,nil);
            }
            //图片保存到相册
        ...
        else if ([mediaType isEqualToString:(NSString *)kUTTypeMovie])
        {
            ...
            if (selfWeak.snycompleted)
            {
                selfWeak.snycompleted(nil,urlStr);
            }
            //视频保存到相册
