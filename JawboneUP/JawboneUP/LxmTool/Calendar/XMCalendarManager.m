...
- (XMCalendarDataSource *)getCurrentCalendarSource {
    ...
    //前面添加一些空的model部位
    [dataSourceModel.dataSource addObjectsFromArray:[self getEmptyCalendarModelWithNumber:self.firstDayWeak - 1 isPreDay:YES]];
    ...
    [dataSourceModel.dataSource addObjectsFromArray:[self getEmptyCalendarModelWithNumber:lastNumber isPreDay:NO]];
...
- (NSArray<XMCalendarModel *>*)getEmptyCalendarModelWithNumber:(NSInteger)number isPreDay:(BOOL)preDay {
    
    NSMutableArray *array = [[NSMutableArray alloc] init];
    
    //前一个月的 第一天
    NSDate * preData = [self.calendarTool getPreMonthFirstDateWithDate:self.firstDate];
    NSArray *dateArray = [self.calendarTool getDatesInMonth:preData];
    for (int i = (int)(dateArray.count - number); i < dateArray.count; i++) {
        NSDate *tmpDate = dateArray[i];
        XMCalendarModel *model = [[XMCalendarModel alloc] init];
        model.date = tmpDate;
        model.chinesDateNumber = [self.calendarTool getChineseDateInDate:tmpDate];
...
//
//    }else{
//        NSDate * nextData = [self.calendarTool getNextMonthFirstDateWithDate:self.firstDate];
//        NSArray *dateArray = [self.calendarTool getDatesInMonth:nextData];
